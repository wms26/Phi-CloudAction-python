...
async def uploadNickname(token):
    async with PhigrosCloud(token) as cloud:
        global nickname
        logger.info(f'玩家昵称："{await cloud.getNickname()}"')  # 获取玩家昵称并输出喵

        if nickname is None or nickname == "":
            nickname = input("请输入你要更改的名称：")

        await cloud.uploadNickname(nickname)
        logger.info("上传昵称成功喵！")


if __name__ == "__main__":
    asyncio.run(uploadNickname(sessionToken))
