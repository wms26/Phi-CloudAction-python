...
class ByteReader:
    ...
    def getByte(self):
        ...
        self.position += 1  # 读取了一个字节喵
        return byte  # 返回读取的单字节数据喵
    ...
    def getVarInt(self):
        ...
        if self.data[self.position] > 127:  # 如果当前字节位置数据的值大于127喵
            ...
            var_int = (self.data[self.position - 2] & 0b01111111) ^ (self.data[self.position - 1] << 7)  # 脑子爆烧唔
        else:
            var_int = self.data[self.position]  # 读取当前字节位置数据的值喵
            self.position += 1  # 向后移动一位喵
        return var_int  # 最后返回读取到的变长整数喵

    def getString(self):
        """读取一段字节并解码为字符串喵"""
        length = self.getVarInt()  # 读当前位置的变长整数喵，代表后续字节长度喵
        string_val = self.data[self.position:self.position + length].decode('utf-8')  # 读取一段字节并uft-8解码喵
        self.position += length  # 根据总字节长度向后移动length位喵
