...
async def ParseGameKey(saveDict: dict | bytes):
    ...
    keySum: int = await Reader.readVarInt()  # 总共key的数量，决定循环多少次喵

    for _ in range(keySum):  # 循环keySum次喵
        name: str = await Reader.readString()  # key的昵称喵
        length: int = await Reader.readByte()  # 总数据长度喵(不包含key的昵称喵)
        Key = all_keys[name] = {}  # 存储单个key的数据喵
        Key['type']: str = str((await readBits(await Reader.readByte()))[:-3])  # 获取key的类型标志喵

        flag: list = []  # 用来存储该key的状态喵(比如头像是否获得/曲绘是否解锁/收藏品是否获取/打开喵)
        for i in range(length - 1):  # 因为前面已经读取了一个类型标志了喵，所以减一喵
            flag.append(await Reader.readByte())
        Key['flag'] = str(flag)

    all_keys['lanotaReadKeys'] = str(await readBits(await Reader.readByte()))  # 读取Lanota收藏品喵(解锁倒霉蛋和船的AT喵)
    all_keys['camelliaReadKey'] = str(await readBits(await Reader.readByte()))  # 读取极星卫破译收藏品喵(解锁S.A.T.E.L.L.I.T.E.的AT喵)
    all_keys['sideStory4BeginReadKey'] = await Reader.readByte()
    all_keys['oldScoreClearedV390'] = await Reader.readByte()
...
async def ParseGameProgress(saveDict: dict | bytes):
    ...
    tem: list = await readBits(await Reader.readByte())  # 鸽游用一个字节表示了下面4个数据喵（
    ...
    all_progress['completed']: int = await Reader.readString()  # 剧情完成喵(显示全部歌曲和课题模式入口喵)
    all_progress['songUpdateInfo']: int = await Reader.readVarInt()  # 喵喵喵？
    all_progress['challengeModeRank']: int = await Reader.readShort()  # 课题分喵

    money: list = []  # 应该不用多说了吧喵（
    for i in range(5):
        money.append(await Reader.readVarInt())
    all_progress['money']: str = str(money)

    all_progress['unlockFlagOfSpasmodic']: str = str((await readBits(await Reader.readByte()))[:-4])  # Spasmodic解锁喵
    all_progress['unlockFlagOfIgallta']: str = str((await readBits(await Reader.readByte()))[:-4])  # Igallta解锁喵
    all_progress['unlockFlagOfRrharil']: str = str((await readBits(await Reader.readByte()))[:-4])  # Rrhar'il解锁喵

    # (倒霉蛋, 船, Shadow, 心之所向, inferior, DESTRUCTION 3,2,1, Distorted Fate, Cuvism)
    all_progress['flagOfSongRecordKey']: str = str(await readBits(await Reader.readByte()))  # 部分歌曲IN达到S喵

    all_progress['randomVersionUnlocked']: str = str((await readBits(await Reader.readByte()))[:-2])  # Random切片解锁喵
    tem: list = await readBits(await Reader.readByte())  # 鸽游用一个字节表示了下面3个数据喵（
    ...
    all_progress['chapter8SongUnlocked']: int = str((await readBits(await Reader.readByte()))[:-2])  # 第八章各曲目解锁喵
    all_progress['flagOfSongRecordKeyTakumi']: int = str((await readBits(await Reader.readByte()))[:-5])
...
async def ParseGameRecord(saveDict: dict | bytes, diff: dict, countRks: bool = True):
    ...
    songSum: int = await Reader.readVarInt()  # 总歌曲数目喵

    for num in range(songSum):
        songName: str = (await Reader.readString())[:-2]  # 歌曲名字喵
        length: int = await Reader.readVarInt()  # 数据总长度喵(不包括歌曲名字喵)
        end_position: int = Reader.position + length  # 单首歌数据结束字节位置喵
        unlock: int = await Reader.readByte()  # 每个难度解锁情况喵
        fc: int = await Reader.readByte()  # 每个难度fc情况喵
        ...
        for level in range(5):  # 遍历每首歌的EZ、HD、IN、AT、Legacy(旧谱喵)难度的成绩喵
            if await readBit(unlock, level):  # 判断当前难度是否解锁喵
                score: int = await Reader.readInt()  # 读取分数喵
                acc: float = await Reader.readFloat()  # 读取acc喵
                try:
...
async def ParseGameSettings(saveDict: dict | bytes):
    ...
    all_settings['deviceName']: str = await Reader.readString()  # 设备名喵
    all_settings['bright']: float = await Reader.readFloat()  # 背景亮度喵
    all_settings['musicVolume']: float = await Reader.readFloat()  # 音乐音量喵
    all_settings['effectVolume']: float = await Reader.readFloat()  # 界面音效音量喵
    all_settings['hitSoundVolume']: float = await Reader.readFloat()  # 打击音效音量喵
    all_settings['soundOffset']: float = await Reader.readFloat()  # 谱面延迟喵
    all_settings['noteScale']: float = await Reader.readFloat()  # 按键缩放喵
...
async def ParseGameUser(saveDict: dict | bytes):
    ...
    all_user['showPlayerId']: int = await Reader.readByte()  # 右上角展示用户id喵
    all_user['selfIntro']: str = await Reader.readString()  # 自我介绍喵
    all_user['avatar']: str = await Reader.readString()  # 头像喵
    all_user['background']: str = await Reader.readString()  # 背景曲绘喵
